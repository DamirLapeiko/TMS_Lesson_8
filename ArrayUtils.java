...
//Например, для массива {1, 2, 3, 4, 5} и индексов 1 и 3 результат будет {2, 3}.
    public static int[] getSubArray(int [] array, int firstIndex, int lastIndex) {
        if (firstIndex < 0 || lastIndex > array.length || firstIndex > lastIndex){
...
//Страница 1 – {1, 2, 3} Страница 2 – {4, 5, 6} Страница 3 – {7}
    public static int[] getSubArrayFromPage(int[] array, int page, int numberOfElements) {
        if (array.length == 0){
            throw new IllegalArgumentException("Error. 0 elements in array");
        }
        if (page < 1){
            throw new IllegalArgumentException("Error. Page number should start with 1");
        }
        if (numberOfElements < 1){
            throw new IllegalArgumentException("Error. Number of elements can't be less 1");
        }
